...
#Обновить ячейку в таблице 
	...
	$stmt->execute(array('cell_value' => $cell_value, 'id_in_db' => $id_in_db));
...
#Пересчитать, записать и заново вывести связанные ячейки	
	...
	$stmt->execute(array('id_in_db' => $id_in_db));
	...
	switch ($column_in_db) {									
		...
		case 'stavka_s_nds':
			...
			$stmt->execute(array('schet' => $schet, 'id_in_db' => $id_in_db));
			//$line_array = $stmt->fetchAll();
		...
		//	$fakticheskij_srok_dostavki = $sdacha - $prinjatie; // 2 - разница в годах(?)
			...
			$stmt->execute(array('fakticheskij_srok_dostavki' => $fakticheskij_srok_dostavki, 'id_in_db' => $id_in_db));
			//$line_array = $stmt->fetchAll();
		...
		case 'prostoj_stavka_za_ohrannika':
			...
			$stmt->execute(array('prostoj_summa' => $prostoj_summa, 'id_in_db' => $id_in_db));
			//$line_array = $stmt->fetchAll();
		...
		case 'arenda_mashin':
			...
			$stmt->execute(array('oplata_mashin' => $oplata_mashin, 'id_in_db' => $id_in_db));
			...
			$stmt->execute(array('itogo' => $itogo, 'id_in_db' => $id_in_db));
		...
		case 'oplata_mashin':
			...
			$stmt->execute(array('zp_plus_prostoj' => $zp_plus_prostoj, 'id_in_db' => $id_in_db));
